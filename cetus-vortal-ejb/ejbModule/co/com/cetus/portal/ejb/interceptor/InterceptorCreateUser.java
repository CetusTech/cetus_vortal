 ...
 */
public class InterceptorCreateUser extends CustomerProcess {
  ...
  public Object interceptService ( InvocationContext ctx ) throws Exception {
    ...
    CreateUserResponseDTO lResponseCreateUSer = null;
    Object[] parameter = null;
    ValidPermServiceRequestDTO validPermDTO = null;
    UserWSDTO userWSDTO = null;
    try {
      lResponseCreateUSer = new CreateUserResponseDTO();
      ...
      if ( !Util.validateResponseSuccess( response ) ) {
        ...
        lResponseCreateUSer.setResponse( createMessageFAILURE_WS() );
        lResponseCreateUSer.getResponse().setMessage( Internalizacion.NO_PERMISSION_SERVICE );
      } else {
        Util.CETUS_CORE.info( "Usuario autorizado para ejecutar el servicio" );
        lResponseCreateUSer = ( CreateUserResponseDTO ) ctx.proceed();
      }
      
    } catch ( Exception e ) {
      Util.CETUS_CORE.error( "Error ::> " + e.getMessage(), e );
      //lResponseCreateUSer.setResponse( createMessageFAILURE_WS() );
      //lResponseCreateUSer.getResponse().setMessage( lResponseCreateUSer.getResponse().getMessage() + "-" + e.getMessage() );
    }
    return lResponseCreateUSer;
  }
